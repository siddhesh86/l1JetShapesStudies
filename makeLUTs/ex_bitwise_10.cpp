...
/*
  To run: g++ ex_bitwise_10.cpp -o ex_bitwise_10 && ./ex_bitwise_10
 */
...
/*
// RawPUS
std::string sInFile_SFs = "LUTs/Default_RawPUS_SF/lut_calib_2024_v0p0_SFGt0_HighGranularity_ECALZS_decimal.txt"; 
std::string sOutFile    = "LUTs/Default_RawPUS_SF/lut_calib_2024_v0p0_SFGt0_HighGranularity_ECALZS.txt"; 
*/

// RawPUS_phiDefault
std::string sInFile_SFs = "LUTs/Default_RawPUS_phiDefault_SF/lut_calib_2024_v0p0_SFGt0_HighGranularity_ECALZS_decimal.txt"; 
std::string sOutFile    = "LUTs/Default_RawPUS_phiDefault_SF/lut_calib_2024_v0p0_SFGt0_HighGranularity_ECALZS.txt"; 
...
unsigned int nCompBinMax = pow(2, nBitsCompBin);  // pow(2, 11) = 2048;  pow(2, 12) = 4096
...
unsigned int calculateSFInBits_addendZero(unsigned int jet_hwPt, double SFInDecimal) {
  ...
  if (printLevel >= 2) {
    std::cout << "multiplier_0: " << multiplier_0 << "\n";
  }

  if (jet_hwPt == 0 && abs(SFInDecimal - 1) < 0.000001) {
    unsigned int addPlusMult = multiplier_0; // multiplier=512 for SFInDecimal==1
    return addPlusMult;
  }
  ...
  if (printLevel >= 2) {
    std::cout << "multiplier_wAddendZero:: ";
    for (size_t i=0; i < multiplier_wAddendZero.size(); i++) {
      std::cout << multiplier_wAddendZero[i] << ",  ";
    }
    std::cout << "\n";
  }
 
  unsigned int addPlusMult;
  ...
  if ( multiplier_wAddendZero.size() > 0 ) {
    ...
    unsigned int jetPtCorr = ((jet_hwPt * multiplier) >> 9) + addend;
    addPlusMult = (addend << 10) + multiplier;
  }
  else {
...
int main ()
{
  ...
  */
  ...
  for (size_t iData=0; iData<data.size(); iData++) {
    ...
    unsigned int compBin       = (etaBin << nBitsPtComp) | ptBin;
    ...
    if ( ptBin == 0) {
      ...
      sComment = "  # compresses ieta bin " + std::to_string(etaBin) + ", pt " + std::to_string(ptBin);  // + " "
    }
